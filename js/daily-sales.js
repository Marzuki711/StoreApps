...
function dsOpenAdd(){if(!dsHasAccess())return;dsEditId="";document.getElementById("dailySalesForm")?.reset();document.getElementById("dsFormTitle").textContent="Add Daily Sales";dsSet("dsDailySalesNo","Auto Generate");dsSet("dsTransactionSize","0.00");dsSet("dsPercentage","0.00%");dsToggleForm(true)}
...
function dsRenderTable(){const b=document.getElementById("dsTableBody"),c=document.getElementById("dsCount");if(!b)return;const q=dsGet("dsSearch").toLowerCase(),rows=dsRows.filter(r=>[r.dsId,r.dailySalesNo,r.storeNo,r.storeName,r.businessDate,r.personInCharge].join(" ").toLowerCase().includes(q));if(c)c.textContent=`${rows.length} Record${rows.length===1?"":"s"}`;if(!rows.length){b.innerHTML='<tr><td colspan="8" class="ds-empty">No Daily Sales records found.</td></tr>';return}b.innerHTML=rows.map(r=>`<tr><td>${dsEsc(r.dsId)}</td><td>${dsEsc(r.dailySalesNo)}</td><td>${dsEsc(r.storeNo)}</td><td>${dsEsc(r.storeName)}</td><td>${dsEsc(r.businessDate)}</td><td class="ds-number">${dsMoney(r.totalSales)}</td><td>${dsEsc(r.personInCharge)}</td><td><button class="ds-edit-btn" type="button" onclick="dsOpenEdit('${dsAttr(r.dsId)}')"><i class="fa-solid fa-pen"></i> Edit</button></td></tr>`).join("")}
